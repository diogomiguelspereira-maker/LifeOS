...
export function Button({
  ...
  const variants: Record<ButtonVariant, string> = {
    ...
    secondary:
      "bg-zinc-100 text-zinc-700 border border-zinc-200/70 shadow-xs hover:bg-zinc-200 dark:bg-white/6 dark:text-zinc-100 dark:border-white/10 dark:hover:bg-white/10 dark:shadow-none",
    ghost:
      "text-zinc-500 hover:text-zinc-800 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:text-zinc-100 dark:hover:bg-white/6",
    outline:
      "border border-zinc-200 text-zinc-700 hover:bg-zinc-50 hover:text-zinc-900 shadow-xs dark:border-white/15 dark:text-zinc-200 dark:hover:bg-white/6 dark:shadow-none",
    danger:
...
export function Card({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-line bg-surface p-5 shadow-card transition-colors",
        className
      )}
      {...props}
    />
...
export function Input({
  ...
  return (
    <input
      className={cn(
        "h-10 w-full rounded-lg border border-line bg-raised px-3 text-sm text-ink placeholder:text-ink-3 shadow-input outline-none transition focus:border-indigo-500/60 focus:ring-2 focus:ring-indigo-500/20",
        className
      )}
      {...props}
    />
...
export function Textarea({
  ...
  return (
    <textarea
      className={cn(
        "w-full rounded-lg border border-line bg-raised px-3 py-2 text-sm text-ink placeholder:text-ink-3 shadow-input outline-none transition focus:border-indigo-500/60 focus:ring-2 focus:ring-indigo-500/20",
        className
      )}
      {...props}
    />
...
export function Select({
  ...
  return (
    <select
      className={cn(
        "h-10 w-full appearance-none rounded-lg border border-line bg-raised px-3 text-sm text-ink shadow-input outline-none transition focus:border-indigo-500/60 focus:ring-2 focus:ring-indigo-500/20 [&>option]:bg-white dark:[&>option]:bg-zinc-900",
        className
      )}
      {...props}
    >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        checked ? "bg-indigo-500" : "bg-zinc-200 dark:bg-white/12"
      )}
    >
      <span
        className={cn(
...
export function Segmented<T extends string>({
  ...
  return (
    <div className={cn("inline-flex max-w-full flex-wrap rounded-lg border border-line bg-raised p-1", className)}>
      {options.map((o) => (
        <button
          key={o.value}
...
export function Modal({
  ...
  const panel = (
    ...
    >
      <div
        className={cn(
          "w-full overflow-y-auto overscroll-contain rounded-xl border border-line bg-surface p-6 shadow-modal animate-scale-in",
          maxWidth
        )}
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-10 text-center">
      <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-zinc-100 text-3xl dark:bg-white/8">{icon}</div>
      <p className="text-sm font-semibold text-ink-2">{title}</p>
      {subtitle && <p className="max-w-xs text-xs text-ink-3">{subtitle}</p>}
    </div>
  );
}
